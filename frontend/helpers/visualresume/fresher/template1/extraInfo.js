...
import RightBlockBulletSmall from "../../../../components/visualresume/fresher/template1/resume/RightBlockBulletSmall"
import BlockBulletSmall from "../../../../components/visualresume/fresher/template1/resume/BlockBulletSmall"
...
const extraInfo = (obj, data, marginSec, marginBullet, marginPage, logo) => {
  ...
  data.value.map((v, i)=>{
    ...
    if(countR > countL){
      ...
      obj.block.props.push({top: rightH + marginBullet, height: height, name: v});
      rightH = rightH + marginBullet + height;
    }else{
      obj.right.components.push(RightBlockBulletSmall);
      obj.right.ids.push(str);
      obj.right.props.push({top: rightH + marginBullet, height: height, name: v});
      rightH = rightH + marginBullet + height;
    }
      ...
      rightH = 297 * (countR -1) + marginPage;
        if(countR > countL) {
          if(countR - 1 == countL){
            ...
            obj.block.components.push(BlockBulletSmall);
            obj.block.ids.push(str);
            obj.block.props.push({top: rightH + marginBullet, height: height, data: v});
            rightH = rightH + marginBullet + height;
        ...
        obj.page1.right.props = obj.right.props;
          ...
          obj.right.components.push(RightBlockBulletSmall);
          obj.right.ids.push(str);
          obj.right.props.push({top: rightH + marginBullet, height: height, data: v});
          rightH = rightH + marginBullet + height;
        }
    }
